...
import SavedAddresses from '~/components/account/SavedAddresses.vue'
...
useHead({
  title: 'حسابي | Nexora',
  meta: [
    { name: 'description', content: 'لوحة تحكم الحساب في متجر Nexora' }
  ]
})
</script>
    ...
    <div class="bg-surface border-b border-border relative overflow-hidden">
      ...
      <div class="container mx-auto px-4 lg:px-8 relative z-10 py-12 md:py-16">
        ...
        <nav class="flex items-center text-sm mb-6 text-text-secondary">
          ...
          <span class="text-text-primary font-medium">حسابي</span>
        </nav>
        
        <div class="max-w-2xl">
          <h1 class="text-3xl md:text-4xl font-extrabold text-text-primary mb-4 tracking-tight">حسابي</h1>
          <p class="text-lg text-text-secondary">
            مرحباً بك مجدداً، هنا يمكنك إدارة طلباتك وبياناتك وتتبع كل ما يخص تسوقك.
          </p>
        </div>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 xl:grid-cols-5 gap-8 items-start">
        ...
        <div class="lg:hidden w-full overflow-x-auto pb-4 -mx-4 px-4 hide-scrollbar">
          <div class="flex gap-2 w-max">
            <!-- Reuse parts of the sidebar logic or render horizontal pills -->
            <NuxtLink to="/account" class="px-5 py-2.5 bg-primary text-white rounded-full text-sm font-bold shadow-sm whitespace-nowrap">لوحة التحكم</NuxtLink>
